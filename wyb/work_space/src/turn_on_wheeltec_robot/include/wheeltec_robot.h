...
//IMU数据结构体
/*
accele_x_data：表示加速度计在 X 轴方向的数据，使用 short 类型存储。short 是一个带符号的短整数类型，通常占用 2 个字节内存空间。
accele_y_data：表示加速度计在 Y 轴方向的数据，同样使用 short 类型存储。
accele_z_data：表示加速度计在 Z 轴方向的数据，同样使用 short 类型存储。
gyros_x_data：表示陀螺仪在 X 轴方向的数据，同样使用 short 类型存储。
gyros_y_data：表示陀螺仪在 Y 轴方向的数据，同样使用 short 类型存储。
gyros_z_data：表示陀螺仪在 Z 轴方向的数据，同样使用 short 类型存储。
*/
...
{
	    uint8_t tx[SEND_DATA_SIZE];	//ROS向下位机发送的数据的长度11
		float X_speed;	       
		float Y_speed;           
		float Z_speed;         
...
{
	    uint8_t rx[RECEIVE_DATA_SIZE];	//下位机发送过来的数据的长度 24
	    uint8_t Flag_Stop;
