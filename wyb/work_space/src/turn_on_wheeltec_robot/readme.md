...
订阅话题cmd_vel进入回调函数Cmd_Vel_Callback
```
Cmd_Vel_Sub     = n.subscribe("cmd_vel",     100, &turn_on_robot::Cmd_Vel_Callback, this); 
```

回调函数功能：速度话题订阅回调函数Callback，根据订阅的指令通过串口发送指令控制下位机
```
...
```
判断是否通过串口接收到新的数据，接受到的话就解析串口数据，发布三个话题
```
...
CMake Error at /opt/ros/noetic/share/catkin/cmake/catkinConfig.cmake:83 (find_package):
  ...
  翻译：
  CMake错误在/opt/ros/noetic/share/catkin/ CMake /catkinConfig。cmake: 83 (find_package):
找不到由“serial”提供的包配置文件
下列名称之一:

serialConfig.cmake
serial-config.cmake

在CMAKE_PREFIX_PATH或set中添加“serial”安装前缀
"serial_DIR"转换为包含上述文件之一的目录。如果“串行”
如果提供了单独的开发包或SDK，请确保已经提供了
安装。

```

## 1.2解决方法：
### 1.2.1.检查有没有安装了 serial 功能包
输入命令：
```
rospack find serial
```
若未安装，则显示：
```
[rospack] Error: package 'serial' not found
```
若已经安装,则显示：
```
/opt/ros/noetic/share/serial
```
### 1.2.2.安装
输入命令：
```.
...
## 3.1 相关资料
ros开发手册8和10  
8.WHEELTEC工程师调试常见Bug汇总编册【2020】.pdf  
10.CH9102使用手册.pdf
## 3.2 设置步骤
1. 把src/turn_on_wheeltec_robot/cfg/wheeltec_controller.rules文件放入/etc/udev/rules.d目录下  
2. service udev reload  
